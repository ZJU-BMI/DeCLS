...
def train_model_mimic_all_cs(train_set, test_set, feature_dims, hidden_size, num_category,
    ...
    while train_set.epoch_completed < epochs:
        ...
        with tf.GradientTape() as tape:
            ...
            for i in range(num_event):
                ...
                denom = 1 - tf.reduce_sum(input_mask1_train_[:,i,:] * predicted_output_, axis=1)  # make subject specific denom.
            ...
            if train_set.epoch_completed % 1 == 0 and train_set.epoch_completed not in logged:
                ...
                for i in range(num_event):
                    ...
                    c_index = concordance_index(ett_test, -predicted_risk_test,I_2)
                ...
                if (train_set.epoch_completed + 1) % 2 == 0:
                    print(
                        '----epoch:{}, whole_loss:{}, contrast_loss:{},contrast_loss2:{},clf_loss:{},gen_loss:{}, c_index:{}'.format(train_set.epoch_completed, whole_loss, contrast_loss_risk, 0,neg_likelihood_loss,
...
def mimic_data_divided(data, visit_len, feature_dims):
    labels = data[:, 40].astype('float32').reshape(-1, visit_len,1)
    features = data[:, 0:40].astype('float32').reshape(-1, visit_len, feature_dims)
    days = data[:, 41].astype('float32').reshape(-1, visit_len)
    ett = data[:, 42].astype('float32').reshape(-1, visit_len,1)
...
def mimic_data_concat(split,features,labels, days, ett, mask1, mask2, mask3, event_num):
    ...
    # return DataSet2(train_features, train_ett, train_labels, train_days),DataSet2(test_features, test_ett, test_labels, test_days)

def mask_divide(label,mask1,mask2,mask3):



    mask1 = [mask1[np.where(label == 0)],mask1[np.where(label == 1)],mask1[np.where(label == 2)],mask1[np.where(label == 3)],mask1[np.where(label == 4)],mask1[np.where(label == 5)]]
    mask2 = [mask2[np.where(label == 0)], mask2[np.where(label == 1)], mask2[np.where(label == 2)],
             mask2[np.where(label == 3)], mask2[np.where(label == 4)], mask2[np.where(label == 5)]]
    mask3 = [mask3[np.where(label == 0)], mask3[np.where(label == 1)], mask3[np.where(label == 2)],
             mask3[np.where(label == 3)], mask3[np.where(label == 4)], mask3[np.where(label == 5)]]
...
def experiment(MASK_RATE,SHUFFLE_RATE):
    ...
    last_meas = data_final_copy[:, -4]  # pat_info[:, 3] contains age at the last measurement
    label = data_final_copy[:, -5]  # two competing risks
    ...
    data_0 = np.asarray(data[data['death_reason'] == 0])
    data_1 = np.asarray(data[data['death_reason'] == 1])
    data_2 = np.asarray(data[data['death_reason'] == 2])
    data_3 = np.asarray(data[data['death_reason'] == 3])
    data_4 = np.asarray(data[data['death_reason'] == 4])
    data_5 = np.asarray(data[data['death_reason'] == 5])
    ...
    features_2, labels_2, days_2, ett_2 = mimic_data_divided(data_2, visit_len, feature_dims)
    features_3, labels_3, days_3, ett_3 = mimic_data_divided(data_3, visit_len, feature_dims)
    features_4, labels_4, days_4, ett_4 = mimic_data_divided(data_4, visit_len, feature_dims)
    features_5, labels_5, days_5, ett_5 = mimic_data_divided(data_5, visit_len, feature_dims)
    features = [features_0,features_1,features_2,features_3,features_4, features_5]
    labels = [labels_0, labels_1, labels_2, labels_3, labels_4, labels_5]
    days = [days_0, days_1, days_2, days_3, days_4, days_5]
    ett = [ett_0, ett_1, ett_2, ett_3, ett_4, ett_5]

    model_type = 'train_model_mimic_all_cs'
    ...
    for i in range(train_repeat):
        ...
        split = mimic_data_spilt(features, labels, 6)
        ...
        for ith_fold in range(k_folds):
            ...
            # test = np.concatenate([test_features, test_labels, test_ett, test_days], axis=2)
            # train = np.concatenate([train_features, train_labels, train_ett, train_days], axis=2)
            # with open('result//train_set_{}.pkl'.format(ith_fold), 'rb') as f:
            #     test = pickle.load(f)
            # with open('result//test_set_{}.pkl'.format(ith_fold), 'rb') as f:
            #     train = pickle.load(f)
            # train_set = DataSet2(train[:,:,1:40], train[:,:,41], train[:,:,40], train[:,:,42])
            # t = np.concatenate([train,test],axis=0)
            # test_set = DataSet2(test[:,:,1:40], test[:,:,41], test[:,:,40], test[:,:,42])

            if model_type == 'train_model_mimic_all_cs':
                # RNN
                c_index1, c_index2, c_index3, c_index4, c_index5,ac = train_model_mimic_all_cs(
                    train_set=train_set,
                    test_set=test_set,
                ...
                c_index1, c_index2, c_index3, c_index4, c_index5 = train_model_mimic_all_cs(
                    train_set=train_set,
                    test_set=test_set,
                    feature_dims=feature_dims,
                    hidden_size=hidden_size,
            ...
            index_list.append([c_index1, c_index2, c_index3, c_index4, c_index5])
        print('epoch  {}-----all_ave  {}'.format(i, np.mean(index_list, axis=0)))
        auc_list = pd.DataFrame(index_list, columns=['c_index1', 'c_index2', 'c_index3', 'c_index4', 'c_index5'])
        auc_list.to_excel('result//pmodel_auc_result_{}_lr={}_3_mask_rate={}_srate={}.xlsx'.format(model_type, learning_rate,MASK_RATE,SHUFFLE_RATE),
                          index=False)
    auc_list = pd.DataFrame(index_list, columns=['c_index1', 'c_index2', 'c_index3', 'c_index4', 'c_index5'])
    auc_list.to_excel('result//pmodel_auc_result_{}_lr={}_3_mask_rate={}_srate={}.xlsx'.format(model_type, learning_rate,MASK_RATE,SHUFFLE_RATE),
                      index=False)
if __name__ == '__main__':
    # experiment()
    # for MASK_RATE in [0.2, 0.4]:
    #     experiment(1,MASK_RATE)
    experiment(1, 1)
